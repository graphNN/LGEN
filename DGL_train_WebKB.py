# features augmentation for graph neural networks on WebKB
...
parser.add_argument('--dataset', type=str, default='film', help='cornell, texas, wisconsin, chameleon')
parser.add_argument('--model', type=str, default='GCN', help='GCN, SGC, GAT, SAGE, GIN, APPNP, TWIRLS')
parser.add_argument('--feat_aug', type=bool, default=False, help='Whether to apply feature enhancement to improve GNN?')
parser.add_argument('--only_aug', type=bool, default=False, help='Whether to apply only feature enhancement to improve GNN?')
parser.add_argument('--other', type=bool, default=False, help='whether other node labels are known?')
...
parser.add_argument('--dataset_split', type=str, default='/diskvdb/rui/mycode/duanrui_0110/second/geom-gcn-master/'
                                                             'geom-gcn-master/splits/',
                        help='')
...
all_test_acc = []
for j in range(10):
    dataset_split = args.dataset_split + f'{args.dataset}_split_0.6_0.2_{j}.npz'
    ...
    for i in range(args.seed, args.seed + 5):
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % 20 == 0:
                ...
                # '||Test loss:{:.4f}'.format(test_loss),||Test accuracy:{:.2f}%'.format(test_acc)'
